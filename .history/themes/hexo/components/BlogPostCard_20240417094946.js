...
const BlogPostCard = ({ index, post, showSummary, siteInfo }) => {
  ...
  return (
        <div className={`${siteConfig('HEXO_POST_LIST_COVER_HOVER_ENLARGE', null, CONFIG) ? ' hover:scale-105 transition-all duration-300' : ''}`} >
            <div key={post.id}
                data-aos="fade-up"
                ...
                className={`group md:h-64 w-full flex justify-between md:flex-row flex-col-reverse ${siteConfig('HEXO_POST_LIST_IMG_CROSSOVER', null, CONFIG) && index % 2 === 1 ? 'md:flex-row-reverse' : ''}
                    overflow-hidden rounded-lg bg-white dark:bg-hexo-black-gray`}>
                ...
                {/* 图片封面 */}
                {showPageCover && (
                    <div className='md:w-5/12 overflow-hidden max-md:border-b-2'>
                        <Link href={`${siteConfig('SUB_PATH', '')}/${post.slug}`} passHref legacyBehavior>
                            <LazyImage priority={index === 1} src={post?.pageCoverThumbnail} className='h-64 w-full object-cover object-center group-hover:scale-110 duration-500' />
                        </Link>
                    </div>
                )}
            </div>
