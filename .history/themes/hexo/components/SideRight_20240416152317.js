    ...
    <div id='sideRight' className='p-2 max-lg:mt-8'>
      {post && post.toc && post.toc.length > 1 &&
        <div className="lg:w-64 flex justify-center items-center mb-4">
          <button
            className={`w-1/2 px-4 py-1 rounded mr-2 ${activeTab === 'toc' ? 'bg-sky-500 text-white' : 'bg-gray-200 text-gray-700'}`}
            onClick={() => setActiveTab('toc')}
          >
            <i className='fas fa-stream' />
          </button>
          <button
            className={`w-1/2 px-4 py-1 rounded ${activeTab === 'info' ? 'bg-sky-500 text-white' : 'bg-gray-200 text-gray-700'}`}
